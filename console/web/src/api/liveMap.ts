...
export type LiveMapMarker = {
  id: number | string;
  type: "player" | "vehicle" | "base" | "storage" | "spice" | "spice_active" | "flour_sand" | "ore" | "scrap" | "flora" | "poi" | "hazard" | "enemy";
  name?: string;
  map?: string;
  partition_id?: number;
  x?: number;
...
export const liveMapApi = {
  ...
  markers: (map = "") => api<{ rows: LiveMapMarker[]; overlays: Record<string, string>; capabilities: Record<string, unknown>; map: LiveMapConfig; maps: Record<string, LiveMapConfig>; defaultMap: string; partitions: LiveMapPartition[]; coriolisSeed?: string }>(`/api/map/markers${map ? `?map=${encodeURIComponent(map)}` : ""}`),
